...
const handelUpdate = ()=>{
  ...
  // use state from category data
  const[categorysfetch,setCategorys] =useState([])


  // getting category data from the databse
      ...
      <div className='container'>
        <form onSubmit={handleFormSubmit}>
          ...
          <div className='mt-3'>
            ...
            {categorysfetch.map((cate,key)=>(
              <option  value={cate.categorys}> {key +1 } {cate.categorys}</option>
            ))}
          ...
          <tbody>
            ...
            {transactions.map((transaction) => (
              <tr key={transaction.id}>
                ...
                <td><button onClick={()=> handelUpdate(transaction.id)} className='btn btn-success ms-lg-5 ' >Update</button></td>
              </tr>
            ))}
          </tbody>
        </table>
